...
class GM:
    ...
    def __init__(self, dt, eta=0.001, eta_d=1., eta_a=0.06, eta_nu=0.01, nu=[1., 1.]):
        ...
        self.Sigma_s_t = np.ones(len(nu))*0.07 #np.array(Sigma_s_t)
    ...
    def update(self, touch_sensory_states, proprioceptive_sensory_states, x):
        ...
        self.nu += -self.dt*self.eta_nu*(-x*self.PE_mu/self.Sigma_mu)  # - 0*x*self.dg_dv(x=self.mu, v=self.dmu)*self.PE_s[1]/self.Sigma_s[1])
