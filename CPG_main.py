...
if __name__ == "__main__":
    dt = 0.005
    n_steps = 25000
    gp = GP(dt=dt, omega2_GP=0.5, alpha=[1., 1.])
    gm = GM(dt=dt, eta=0.001, eta_d=1., eta_a=0.06, eta_nu=0.01, nu=gp.a)#, x=gp.x[0], eta=0.001, eta_d=1., eta_a=0.06, eta_nu=0.01, omega2_GM=0.5, nu=1)
    ...
    for step in np.arange(n_steps):
        ...
        cpg.append( gp.cpg[0] )
        obj_position.append( [gp.object_position[0], gp.object_position[1]] )
    ...
    plt.plot(time, obj_position[:,0], c="black", label=r"object")
    ...
    plt.plot(time, obj_position[:,1], c="black", label=r"object")
    #plt.plot(time, cpg, label=r"cpg")
    #plt.ylim(-1.5,2.5)
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.)
    plt.subplot(212)
    plt.plot(time, gm_mu[:,1], c="green", label=r"$\mu_1$")
    plt.plot(time, gm_nu[:,1], c="#66aa66", label=r"$\nu_1$")
    plt.plot(time, obj_position[:,1], c="black", label=r"object")
    #plt.plot(time, gm_dmu[:,0], label=r"$\mu_0'$")
    #plt.plot(time, cpg, label=r"$cpg$")
    #plt.plot(time, gm_nu[:,0], label=r"$\nu_0$")
